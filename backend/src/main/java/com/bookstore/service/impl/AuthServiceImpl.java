...
public class AuthServiceImpl implements AuthService {
    ...
    public Map<String, Object> shopLogin(LoginDTO dto) {
        ...
        if (shop == null) {
            throw new BusinessException("您还不是商家，请先注册");
        }

        // 检查店铺状态
        if (shop.getStatus() == 2) {
            throw new BusinessException("店铺已被禁用");
        }
        if (shop.getStatus() == 0) {
            throw new BusinessException("店铺审核中，请耐心等待");
        }
        if (shop.getStatus() == 3) {
            throw new BusinessException("店铺审核未通过：" + shop.getAuditRemark());
        }
